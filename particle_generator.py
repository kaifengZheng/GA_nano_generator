...
def calculate_similarity(ind1, ind2):
    ...
    dist = np.max(cdist(np.array(ind1), np.array(ind2)))
    return dist
...
def genetic_algorithm(
    ...
    for generation in tqdm(range(generations)):
        ...
        print(f"len(population)={len(populations)}")
        # population length control
...
if __name__ == "__main__":
    ini_configurations = {
        ...
        "mutation_rate": 0.4,
        "cross_over_rate": 0.3,
        "elite_fraction": 0.005,
        "descriptors": {
            "diameter_2radius": 11.31,
            "surface ratio": 0.86,
            "atom_number": 44,
        },
    }
